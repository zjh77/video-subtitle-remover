...
def download_with_resume(dst:Path,size:int,sha:str,open_response:Callable[[int],object],progress=None)->Path:
 ...
 if n==size:
  if sha256_file(part).lower()==sha.lower():os.replace(part,dst);return dst
  part.unlink();n=0
 r=open_response(n)
 try:
  ...
  with part.open('ab' if n else 'wb') as f:
   ...
   except (OSError,http.client.HTTPException,ssl.SSLError,TimeoutError,socket.timeout) as exc:raise RelayTransientError('artifact download stream interrupted') from exc
 finally:r.close()
